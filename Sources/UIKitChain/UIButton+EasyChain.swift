...
public extension EasyChain where Base: UIButton {
    ...
    @available(iOS 13.4, *)
    @discardableResult func pointerInteractionEnabled(_ pointerInteractionEnabled: Bool) -> Self {
    ...
    @discardableResult func setTitle(_ title: String?, for state: UIControl.State) -> Self {
        return make { $0.setTitle(title, for: state) }
    }
        
    @discardableResult func setTitleColor(_ color: UIColor?, for state: UIControl.State) -> Self {
        return make { $0.setTitleColor(color, for: state) }
    }
    
    @discardableResult func setTitleShadowColor(_ color: UIColor?, for state: UIControl.State) -> Self {
        return make { $0.setTitleShadowColor(color, for: state) }
    }

    @discardableResult func setImage(_ image: UIImage?, for state: UIControl.State) -> Self {
        return make { $0.setImage(image, for: state) }
    }
    
    @discardableResult func setBackgroundImage(_ image: UIImage?, for state: UIControl.State) -> Self {
        return make { $0.setBackgroundImage(image, for: state) }
    }
    
    @available(iOS 13.0, *)
    @discardableResult func setPreferredSymbolConfiguration(_ configuration: UIImage.SymbolConfiguration?, forImageIn state: UIControl.State) -> Self {
        return make { $0.setPreferredSymbolConfiguration(configuration, forImageIn: state) }
    }
    
    @available(iOS 6.0, *)
    @discardableResult func setAttributedTitle(_ title: NSAttributedString?, for state: UIControl.State) -> Self {
